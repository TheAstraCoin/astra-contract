	...
	*/
	...
	contract astraHelper is Context, Ownable {
		...
		address public astraToken = address(0);
		...
		function setastraAddress(address newastra) public onlyOwner returns(bool) {
			require(astraToken == address(0), "astra already set up");
			astraToken = newastra;
			isWhitelisted[astraToken] = true;
			
			return true;
		}
		
		function setastraPairAddress(address newastraPair) public onlyOwner returns(bool) {
			require(_lgePairAddress == address(0), "astra Pair already set up");
			_lgePairAddress = newastraPair;
		...
		function engageMechanicsOnTransfer(address sender, address recipient, uint256 amount) external returns(bool) {
			...
			require(msg.sender == astraToken, "Message sender is not astra Token");
			...
			if(isWhitelisted[toCooldown] == false) {
				require(cooldown[toCooldown] < now, "astra Cooldown: user cooldown active");
